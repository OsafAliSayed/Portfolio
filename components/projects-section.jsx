          ...
          {projects.map((project, index) => (
            <Dialog key={index}>
              <DialogTrigger asChild>
                ...
                >
                  <Card className="h-full overflow-hidden border border-border hover:border-tertiary/50 transition-all duration-300 hover:shadow-md relative group">
                    <div className="w-full h-full overflow-hidden">
                      ...
                      <div className="absolute inset-0 project-gradient-overlay flex flex-col justify-end p-4">
                        <div className="transform translate-y-3 group-hover:translate-y-0 transition-transform duration-300">
                          <div className="flex justify-between items-start mb-2">
                            <h3 className="text-lg font-bold text-secondary line-clamp-1">{project.title}</h3>
                            {project.status && (
                              <span className="px-2 py-0.5 bg-tertiary-transparent-10 text-secondary rounded-full text-xs font-medium whitespace-nowrap ml-2">
                                {project.status}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {project.technologies.slice(0, 2).map(tech => (
                              <span key={tech} className="inline-block px-2 py-0.5 bg-tertiary text-primary rounded-md text-xs backdrop-blur-sm">
                                {tech}
                              </span>
                            ))}
                            {project.technologies.length > 2 && (
                              <span className="inline-block px-2 py-0.5 bg-tertiary text-primary rounded-md text-xs backdrop-blur-sm">
              ...
              <DialogContent className="sm:max-w-5xl md:max-w-6xl lg:max-w-7xl p-0 overflow-hidden" showX={false}>
                <DialogClose className="absolute right-6 top-6 rounded-full w-10 h-10 flex items-center justify-center bg-primary/90 hover:bg-primary shadow-md z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2">
                  ...
                  <div className="p-6 flex flex-col h-full overflow-y-auto max-h-[85vh] hide-scrollbar">
                    ...
                    <div className="mt-8">
                      <h3 className="text-base font-medium text-text-dark mb-3">Technologies</h3>
                      <div className="flex flex-wrap gap-2 mb-8">
                        {project.technologies.map(tech => (
                          <span key={tech} className="inline-block px-3 py-1.5 bg-tertiary/10 text-tertiary rounded-md text-sm">
                            {tech}
                          </span>
                        ))}
                      </div>
                      
                      <h3 className="text-base font-medium text-text-dark mb-3">Description</h3>
                      <p className="text-base leading-relaxed text-foreground mb-8">
                        {project.longDescription || project.description}
                      </p>
                      
                      <div className="flex space-x-5 mt-auto pt-6">
                        <Button variant="outline" size="default" asChild className="border-tertiary hover:bg-tertiary/10 text-text hover:text-text">
                        ...
                        {project.live && (
                          <Button size="default" asChild className="bg-tertiary hover:bg-tertiary/90 text-primary">
                  ...
                  <div className="bg-quaternary/10 flex flex-col h-full max-h-[85vh] overflow-y-auto hide-scrollbar">
                    {project.images && project.images.length > 0 ? (
                      <div className="w-full p-6">
                        <div className="grid grid-cols-1 gap-8 w-full">
                          {project.images.map((image, imageIndex) => (
                            <div 
                              key={imageIndex} 
                              className="w-full rounded-lg overflow-hidden shadow-lg border border-tertiary/20"
                            >
                              <img 
                                src={image} 
                      ...
                      <div className="flex items-center justify-center h-full w-full">
                        <p className="text-text-dark">Images coming soon</p>
                      </div>
                    )}
                  </div>
                </div>
